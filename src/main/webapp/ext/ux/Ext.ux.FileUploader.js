 ...
 * @version $Id: Ext.ux.FileUploader.js,v 1.1 2010/02/20 09:53:24 lphuang Exp $
 * @date    15. March 2008
 *
...
Ext.extend(Ext.ux.FileUploader, Ext.util.Observable, {
    ...
    ,setServerFileName: function(record, o) {
        // may be returns the server filename?
        var fs = o.filenames, value = (fs != null) ? fs[record.id] : null;
        record.set('serverFileName', value != null ? value : record.get('serverFileName'));
    }
	...
	,processSuccess:function(options, response, o) {
		...
		if(this.singleUpload) {
			...
			record.set('error', '');
            this.setServerFileName(record, o);
			record.commit();
		}
		...
		if(this.singleUpload) {
			...
			records.each(function(record) {
				...
				if(e) {
					...
					record.set('state', 'done');
					record.set('error', '');
                    this.setServerFileName(record, error);
				}
				record.commit();
			}, this);
