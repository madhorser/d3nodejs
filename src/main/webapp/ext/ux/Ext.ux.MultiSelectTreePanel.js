...
//If you are using Ext-JS 2.2.1 or earlier you need to add this override! 
//(reported as a bug should be fixed in the next version)
//@link{http://www.users.on.net/~clear/ext/dev-1.2-alpha/}
Ext.override(Ext.tree.TreeDropZone, {
    completeDrop : function(de) {
        var ns = de.dropNode, p = de.point, t = de.target;
        if(!Ext.isArray(ns)) {
        ...
        if (n) {
            if (n.ui && n.ui.rendered) {n.ui.focus();}
            if(Ext.enableFx && this.tree.hlDrop) {
                if (n.ui && n.ui.rendered) {
                    n.ui.highlight();
                }
            }
        }
